...
namespace C64GBOnline.Infrastructure
{
    public sealed class JsonRepository<T> : IRepository<T>
    {
        ...
        public JsonRepository(string repositoryFullName) => _repositoryFullName = repositoryFullName;
        ...
        {
            if (!_isInitialized) throw new InvalidOperationException("Not initialized");
            await using FileStream fileStream = new FileStream(_repositoryFullName, FileMode.Create, FileAccess.Write, FileShare.Write, 4096, FileOptions.Asynchronous);
            await JsonSerializer.SerializeAsync(fileStream, _dictionary.Values.ToList(), new() { WriteIndented = true });
        }
        ...
        public async Task Initialize(Expression<Func<T, object>> idColumn)
        {
            ...
            try
            {
                ...
                await using FileStream fileStream = new FileStream(_repositoryFullName, FileMode.Open, FileAccess.Read, FileShare.Read, 4096, FileOptions.Asynchronous | FileOptions.SequentialScan);
                var _list = await JsonSerializer.DeserializeAsync<List<T>>(fileStream) ?? new();
